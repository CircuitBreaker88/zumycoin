...
*
*   Why is this needed?
*   - In Zumy some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    if(nBlockHeight < consensusParams.nSuperblockStartBlock) {
        ...
        // Superblocks are disabled for Zumy.
        int nOffset = nBlockHeight % consensusParams.nBudgetPaymentsCycleBlocks;
        if(nBlockHeight >= consensusParams.nBudgetPaymentsStartBlock &&
            nOffset < consensusParams.nBudgetPaymentsWindowBlocks) {
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, CTxOut& txoutMasternodeRet)
{
    ...
    CAmount devPayment = 0.21 * COIN; // Developer Gov fund amount
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if(fLiteMode) return; // disable all Zumy specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f ZMY\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
