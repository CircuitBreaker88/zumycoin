...
#if defined(NDEBUG)
# error "Zumy cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Zumy Signed Message:\n";
...
CAmount GetPoWBlockPayment(const int& nHeight, CAmount nFees)
{
    ...
    else if (chainActive.Height() >= 2 && nNextHeight <= Params().GetConsensus().nZumyBeginsBlock) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, const bool fWriteNames)
{
    ...
    // ZMY : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if(!IsBlockValueValid(block, pindex->nHeight, nExpectedBlockValue, strError)){
        return state.DoS(0, error("ConnectBlock(ZMY): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, nExpectedBlockValue)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ZMY): couldn't find Masternode or Superblock payments"),
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode) {
    ...
    size_t cacheSize = pcoinsTip->ZumyMemoryUsage();
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("%s: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%.1fMiB(%utx)\n", __func__,
      ...
      Checkpoints::GuessVerificationProgress(chainParams.Checkpoints(), chainActive.Tip()), pcoinsTip->ZumyMemoryUsage() * (1.0 / (1<<20)), pcoinsTip->GetCacheSize());
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
         ...
         BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ZMY): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ZMY): spork is off, skipping transaction locking checks\n");
    }
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (coins.ZumyMemoryUsage() + pcoinsTip->ZumyMemoryUsage()) <= nCoinCacheUsage) {
            bool fClean = true;
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Zumy Related Inventory Messages
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::PSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            LogPrint("mempool", "AcceptToMemoryPool: peer=%d: accepted %s (poolsz %u txn, %u kB)\n",
                pfrom->id,
                tx.GetHash().ToString(),
                mempool.size(), mempool.ZumyMemoryUsage() / 1000);
